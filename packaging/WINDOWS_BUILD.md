...
2. Docling 已改为独立共用资产，不再嵌入程序基座。用 `scripts/prepare_windows_models.py --manifest src/knowledge_distiller/v1/adapters/docling-models-manifest.json --output .windows-build/docling-models` 下载固定修订并校验全部30文件。Paddle用 `scripts/prepare_paddle_models.py --output .windows-build/paddle-models`，保留其生成清单。
