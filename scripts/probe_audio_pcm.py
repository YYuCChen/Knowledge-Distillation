...
def main():
    ...
    result = {'source_sha256': hashlib.sha256((args.fixtures/'source.m4a').read_bytes()).hexdigest(),
              ...
              'normalization_input_name': 'standard.wav',
              'normalization_input_sha256': hashlib.sha256((args.fixtures/'standard.wav').read_bytes()).hexdigest(),
              'cases': cases,
              'compressed': {'input': 'source.m4a',
                             'route': 'source.m4a -> independent fixed-decoder baseline -> product normalization and previews',
