...
Windows ASR 从原始解码移除语言协议头，保留正文中的重复词句。Transformers 的 parsed 后处理会压缩长重复，不能作为来源文本。若后处理会改写正文，或输出有连续五份相同词句，即使模型返回 EOS，也进入现有分段恢复；短段仍需正常结束，未恢复完成不能建立完整来源事实。此策略不根据预期答案补词，也不证明任意语音均无漏识。worker 内容哈希参与缓存身份，解析或资格规则变更后旧识别缓存不会继续生效。
